...
?>
                ...
                <form name="lessions" id="lessions" action="<?=base_url().'Lession/'?><?php if ($this->uri->segment('2') == "editLession") {
                  ...
                  <input type="hidden" name="hidden_course_id" value="<?=$lession['course_id']?>">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <select class="form-control" name="course_id" id="course_id" required onchange="check_course(0)">
                  ...
                  <div class="form-group div_lession_type" id="div_lession_type" <?php if ($this->uri->segment('2') != "editLession"){?> style="display: none;" <?php } ?>>

                    <label for="title" class="col-sm-2 control-label">Chapter Type</label>

                    <div class="col-sm-10">
                        <input type="radio" name="lession_type" id="normal_lession" value="normal" onclick="lession_training_details('normal');" <?= isset($lession) && $lession['lession_type'] == 'normal' ?'checked':'' ?>>    Normal 
                          <input type="radio" name="lession_type" id="training_lession" value="training" style="margin-left: 15px;margin-top: 10px" onclick="lession_training_details('training');" <?= isset($lession) && $lession['lession_type'] == 'training' ?'checked':'' ?> <?php if(!isset($lession) || $lession['lession_type'] == null){?> checked <?php }?>> Training
                    </div>

                  </div>

                  <div id="lession_training_type" <?php if((isset($lession) && $lession['lession_type'] == 'training') || (isset($lession) && $lession['lession_type'] == null)){ ?> style="display: block;" <?php }else{ ?>style="display: none;" <?php } ?> >
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-5" id="div_ear_type" style="display: block;">
                          <input type="radio" name="ear_training_type" value="all" <?= isset($lession) && $lession['ear_training_type'] == 'all' ?'checked':'' ?> <?php if(!isset($lession) || $lession['ear_training_type'] == null){?> checked <?php }?>>    All notes up and down 
                            <input type="radio" name="ear_training_type" value="four" <?= isset($lession) && $lession['ear_training_type'] == 'four' ?'checked':'' ?> style="margin-left: 15px;margin-top: 10px" >    4 chords (1,5,6,4)
                      </div>
                    </div>
                  ...
                  <div id="div_free_for_all" <?php if(isset($lession) && $lession['course_id'] != ''){?> style="display:block" <?php }else{?> style="display:none" <?php }?> >
                  <div class="form-group">
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    // var hid_course = $("#hidden_course_id").val();
    // check_course(hid_course);
  ...
  function viewspeedTraining(id) {
    ...
    // var training_url = 'http://18.235.99.234/piano/APIs/uploads/chapter/'; //Live 
    $.ajax({
      type: "GET",
  ...
  function check_course(course_id) {
    ...
    $.ajax({
      type: "GET",
      url: '<?= base_url() ?>Lession/lession_view/' + course_id,
      data: {},
      cache: false,
      'Content-Type': "application/json",
      success: function (data) {
        var result = (JSON.parse(data));
        document.getElementById('div_lession_type').style.display = "block";
        document.getElementById('div_free_for_all').style.display = "block";

        if (result.course_type == 'hearing') {
          $("#training_lession").prop("checked", true);
        } else{
          $("#normal_lession").prop("checked", true);  
        }

        var lession_type = $('input[name="lession_type"]:checked').val();
        lession_training_details(lession_type)
        
      }
    });
  }

  function lession_training_details(lession_type) {
    if (lession_type == 'training') {
      document.getElementById('lession_training_type').style.display = "block";
    }else{
      document.getElementById('lession_training_type').style.display = "none";
    }
  }
