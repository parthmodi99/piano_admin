...
class Admin_model extends CI_Model
{
    ...
    function get_no_of_exercise($id)
    {
        $this->db->select('*');
        $this->db->from("tbl_ear_score");
        $this->db->where('user_id', $id);
        // $this->db->where('correct_answer', 20);
        $result = $this->db->get();
        return $count = $result->num_rows();
    }
    ...
    public function getAllChatList(){
        $sel = $this->db->query("select from_id from tbl_chat where to_id=1 union select distinct to_id from tbl_chat where from_id = 1");
        if ($sel->num_rows() > 0) {
            $data = $sel->result_array();
            $list = array();
            foreach($data as $chat){
                $last_message = $this->db->query("SELECT * FROM tbl_chat WHERE from_id = '".$chat['from_id']."' AND to_id=1 OR from_id = 1 AND to_id='".$chat['from_id']."' ORDER BY id DESC LIMIT 0,1");
                if ($last_message->num_rows() > 0) {
                    $message = $last_message->row_array();
                    $user_detail = $this->db->query("SELECT * FROM tbl_user WHERE id='".$chat['from_id']."'");
                    if ($user_detail->num_rows() > 0) {
                        $user_data = $user_detail->row_array();
                        $message['user_name'] = $user_data['name'];
                        $message['user_id'] = $chat['from_id'];
    ...
    public function getAllConversationList($user_id){
        $sel = $this->db->query("SELECT * FROM `tbl_chat` WHERE from_id = ".$user_id." OR to_id = ".$user_id."");
        if ($sel->num_rows() > 0) {
            return $sel->result_array();
        } else {
            return array();
