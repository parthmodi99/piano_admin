...
{
    // public $chapter_url = 'http://18.235.99.234/piano/APIs/uploads/chapter/'; //live
    ...
    public function addLession()
    {
        ...
        if ($this->input->post('submit')) {
            ...
            $max_po = $this->db->query("SELECT MAX(position) as max_position FROM tbl_lession");
            $position = $max_po->row_array();
            ...
            $insert_data['lession_type'] = $this->input->post('lession_type', true) ;

            if($insert_data['lession_type'] == 'training'){
                ...
                $insert_data['ear_training_type'] = $insert_data['ear_training'] == 1 ? $this->input->post('ear_training_type', true) : '';
                $insert_data['free_for_all'] = $this->input->post('free_for_all', true) ;
            }

            
            
            $insert_data['position'] = $position['max_position'] + 1;  
                ...
                if ($result) {
                    ...
                    for ($i = 0; $i < $cpt; $i++) {
                        ...
                        if (in_array($imageFileType, $image)) {
                            ...
                            $destination = '/var/www/html/piano/APIs/uploads/chapter/convert_' . $dataInfo['raw_name'] . '.mp4';
        ...
        if ($this->input->post('submit')) {
            ...
            if ($lession_id != '') {
                ...
                if($insert_data['lession_type'] == 'training'){
                    ...
                    $insert_data['ear_training'] = $this->input->post('ear_training', true) ? $this->input->post('ear_training', true) : '0';
                    $insert_data['recognizing'] = $this->input->post('recognizing', true) ? $this->input->post('recognizing', true) : '0';
                    $insert_data['ear_training_type'] = $insert_data['ear_training'] == 1 ? $this->input->post('ear_training_type', true) : null;
                }else{
                    $insert_data['speed_training'] = 0;
                    $insert_data['unlimited_key_training'] = 0;
                    $insert_data['ear_training'] = 0;
                    $insert_data['recognizing'] = 0;
                    $insert_data['ear_training_type'] = null;
                    // $insert_data['free_for_all'] = 0;
                }
                $insert_data['free_for_all'] = $this->input->post('free_for_all', true);
                ...
                if ($insert_data['speed_training'] == 1) {
                    ...
                    if ($upd) {
                        ...
                        for ($i = 0; $i < $cpt; $i++) {
                            ...
                            $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION));
                            // echo "<pre>";
                            // print_r($_FILES["reco_media"] ["size"]); 
                            // define('MB', 1048576);
                            // if ($_FILES['reco_media']['size'] < 113*MB){
                            //     echo "->in";
                            // }else{
                            //     echo "->out";
                            // }
    ...
    public function lession_view($id)
    {
        $data = $this->admin_model->getSingleRecordById('tbl_course', array('id' => $id));
        echo json_encode($data);
    }
}
