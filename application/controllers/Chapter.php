...
class Chapter extends CI_Controller
{
    ...
    public function addChapter()
    {
        ...
        if ($this->input->post('submit')) {
            $max_po = $this->db->query("SELECT MAX(position) as max_position FROM tbl_chapter");
                ...
                if ($data['ear_training'] == 1) {  
                    ...
                    if (isset($_FILES['letter_image']) && $_FILES['letter_image']['name'] != '') {
                        ...
                        if (in_array($imageFileType, $image)) {
                            ...
                            $destination = '/var/www/html/piano/APIs/uploads/chapter/convert_' . $dataInfo1['raw_name'] . '.mp4';
                    ...
                    for ($i = 0; $i < $cnt; $i++) {
                        ...
                        if ($i == 0) {
                            if($data['ear_training_type'] == 'all'){
                                $key_name = 'all_slow';
                            }else if($data['ear_training_type'] == 'four'){
                                $key_name = 'four_slow';
                            }   
                        } elseif ($i == 1) {
                            if($data['ear_training_type'] == 'all'){
    ...
    */
    ...
    public function editChapter($id)
    {
        ...
        $data['is_ear_training_type'] = $data['chk_lession']['ear_training_type'];
        $data['is_recognizing'] = $data['chk_lession']['recognizing'];

        if($data['chapter']['position'] == null){
            $max_po = $this->db->query("SELECT MAX(position) as max_position FROM tbl_chapter");
            $position = $max_po->row_array();
            $insert_data['position'] = $position['max_position'] + 1;    
        }
                ...
                if ($upd) {
                    if ($lession_details['ear_training'] == 1) {
                        ...
                        for ($i = 0; $i < $cnt; $i++) {
                            ...
                            if ($i == 0) {
                                if($lession_details['ear_training_type'] == 'all'){
                                    $key_name = 'all_slow';
                                }else if($lession_details['ear_training_type'] == 'four'){
                                    $key_name = 'four_slow';
                                }   
                            } elseif ($i == 1) {
                                if($lession_details['ear_training_type'] == 'all'){
