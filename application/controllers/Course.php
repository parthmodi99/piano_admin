...

{
...

    {
        ...
        if($this->input->post('submit'))
        {            
            $max_po = $this->db->query("SELECT MAX(position) as max_position FROM tbl_course");
            $position = $max_po->row_array();
            
            $insert_data['name'] = $this->input->post('name',TRUE);
            $insert_data['coming_soon'] = $this->input->post('coming_soon', true) ? $this->input->post('coming_soon', true) : '0';
            $insert_data['course_type'] = $this->input->post('course_type', true);
            ...
            if (isset($_FILES['feature_image']) && $_FILES['feature_image'] != '') {
                $config = array(
                    'upload_path' => "../APIs/uploads/chapter/", //live
                    // 'upload_path' => "upload/chapter/", //local
                    'allowed_types' => "gif|jpg|png|jpeg|pdf",
                ...
                if ($this->upload->do_upload('feature_image')) {
                    $data = $this->upload->data();
                    $insert_data['feature_image'] = $data['file_name'];
                }
            }
